...
# 1. Creer la NACL et recuperer son ID depuis nacl.json
create_nacl "$1"

# 2. Lire ID NACL depuis nacl.json
NACL_ID_FROM_FILE=$(grep -o '"NetworkAclId": "[^"]*"' nacl.json | cut -d'"' -f4)
echo "ID NACL recuperer depuis nacl.json : $NACL_ID_FROM_FILE"

# 3. Creer la regle entrante en passant ID recupere
create_ingress_rule "$NACL_ID_FROM_FILE" 100 6 443 443 "deny"

# 4. Creer la regle sortante en passant ID recupere
create_egress_rule "$NACL_ID_FROM_FILE" 100 6 443 443 "deny"

# 5. Verification
VERIFICATION=$(aws ec2 describe-network-acls --network-acl-ids "$NACL_ID_FROM_FILE")
