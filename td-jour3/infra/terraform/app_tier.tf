...
resource "aws_lb" "internal" {
  ...
  internal           = true # ALB INTERNE, pas d'IP publique
...
resource "aws_instance" "app" {
  ...
  user_data = templatefile("${path.module}/app/user_data.sh.tpl", {
    ...
    # Injection du code source sans git clone
  ...
  # Dépendance de NAT GW
  depends_on = [aws_db_instance.postgres, aws_nat_gateway.nat]
}
